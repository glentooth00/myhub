...
function getAllocsBreakdown( $app, $allocs, $validate = false )
{
  ...
  foreach( $allocs as $tradeUID => $coverAmount ) {
    if ( $tradeUID != '_FIX_' ) $tradeUIDs[] = $tradeUID;
...
if ( $app->request->isPost ) {
  ...
  try {
    ...
    if ( $action == 'saveTcc' ) {
      ...
      if ( ! is_array( $updatedAllocs ) ) $updatedAllocs = [];
      ...
      if ( $allocatedTradesJson != $tccBefore->allocated_trades ) {
        ...
        if ( ! is_array( $currentAllocs ) ) $currentAllocs = [];
        ...
        foreach( $updatedAllocs as $tradeId => $coverAmount ) {
          ...
          if ( ! $trade ) throw new Exception( "Removed trade $tradeId not found in current allocated trades." );
          $tradePins = json_decode( $trade->allocated_pins ?: '{}', true );
          if ( ! is_array( $tradePins ) ) $tradePins = [];
      ...
      if ( $isNew and ! $saveTccResult ) throw new Exception( 'Failed to save new TCC.' );
      ...
      if ( ! $tcc or ! $tcc->id ) throw new Exception( "TCC $savedTccId not found." );
...
if ( $isNew ) $tcc->client_id = $_GET['client'] ?? '';
