F1.deferred.push(function initBeneficiaryListPageView(app) {

    console.log('initBeneficiaryListPageView()');
  
    const Utils = F1.lib.Utils;
  
    /* table */
    const table = Utils.getEl('beneficiary-table');
    table.onclick = function(event) {
      console.log('table.onclick:', event);
      if (event.target.tagName.toLowerCase() === 'button') return;
      const row = event.target.closest('tr');
      if (!row || !row.id || row.parentElement.tagName.toLowerCase() !== 'tbody') return;
      app.navigateTo({url:'admin/referrers?view=beneficiary-details&id='+row.id, view:'beneficiary-details', title:'Beneficiary Details'});
    };
    ...
    const searchControl = Utils.getEl('beneficiary-table_filter');
  ...
  }); // END: initBeneficiaryListPageView