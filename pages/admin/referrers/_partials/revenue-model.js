F1.deferred.push(function initRevenueListPageView(app) {

    console.log('initRevenueListPageView()');
  
    const Utils = F1.lib.Utils;
  
    /* table */
    const table = Utils.getEl('revenue-table');
    table.onclick = function(event) {
      console.log('table.onclick:', event);
      if (event.target.tagName.toLowerCase() === 'button') return;
      const row = event.target.closest('tr');
      if (!row || !row.id || row.parentElement.tagName.toLowerCase() !== 'tbody') return;
      app.navigateTo({url:'admin/referrers?view=revenue-details&id='+row.id, view:'revenue-details', title:'Revenue Details'});
    };
    ...
    const searchControl = Utils.getEl('revenue-table_filter');
