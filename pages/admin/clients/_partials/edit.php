...
if ($app->request->isPost) {
  ...
  try {
    ...
    if ($action == 'addOption') {
      ...
      if (!$table)
        throw new Exception('Invalid select field: ' . $fieldName);

      $optName = $_POST['optName'] ?? '';
      if (!$optName)
        throw new Exception('Option name undefined or invalid.');
      ...
      if ($table == 'ch_referrers') {
        ...
        if (isset($_POST['extraData'])) {
          ...
          if (isset($extraData['id_number']))
            $newOption['id_number'] = $extraData['id_number'];
          if (isset($extraData['email']))
            $newOption['email'] = $extraData['email'];
          if (isset($extraData['phone_number']))
            $newOption['phone_number'] = $extraData['phone_number'];
        }
      }

      $insertResult = $app->db->table($table)->insert(
        $newOption,
    ...
    if ($action == 'updateOption') {
      ...
      if ($existingOption)
        throw new Exception("Another option [$newOptName] already exists.");
      ...
      if (!$option)
        throw new Exception("Option [$optName] not found.");
      ...
      $updateResult = $app->db->table($table)->update(
        (array) $option,
        ['autoStamp' => true, 'user' => $app->user->user_id]
      );
    ...
    if ($action == 'saveClient') {
      ...
      if (required($idNumber, 'always'))
        throw invalidMessage('id_number.required');
      ...
      if ($referrerId) {
        $referrer = $app->db->getFirst('ch_referrers', $referrerId);
        if (!$referrer)
          throw new Error("Referrer id=$referrerId not found.");
        $_POST['inhouse_referrer_15_percent'] = $referrer->user_id;
        $_POST['third_party_referrer'] = $referrer->name;
      }
      ...
      if ($isNew and !$saveResult)
        throw new Exception('Failed to save new client.');
      ...
      if ($isNew or $clientHasRelevantChanges) {
        ...
        $payload = [
          'sheetName' => __GOOGLE_CLIENTS_SHEET_NAME__,
      ...
      $clientData['spare_1'] = process_upload(
        'spare_1_file',
        $clientBefore->spare_1 ?? '',
        $marriageCert,
        $saveDir,
        'spare_1'
      );

      $clientData['spare_2'] = process_upload(
        'spare_2_file',
        $clientBefore->spare_2 ?? '',
        $cryptoDecl,
        $saveDir,
        'spare_2'
      );
...
$suburbs = $app->db->table('suburbs')->orderBy('name')->getAll();
$provinces = $app->db->table('provinces')->orderBy('name')->getAll();
$countries = $app->db->table('countries')->orderBy('name')->getAll();
