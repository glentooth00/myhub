...
if ( $app->request->isPost ) {
  ...
  try {
    ...
    if ( $action === 'deleteSuburb' )
    {
      ...
      $db->table('loc_suburbs')->delete( $id );
      
      $db->pdo->commit();

      json_response( [ 'success' => true, 'message' => 'Suburb deleted.' ] );
    ...
    //   $report['type_id'] = $_POST['type_id'] ?? null;
      ...
      $result = $db->table( 'loc_suburbs' )->save( $report, ['autoStamp' => true, 'user' => $uid ] );
      debug_log( $result, 'Save Suburb - Result: ' );
      
      $db->pdo->commit();

      json_response( [ 'success' => true, 'message' => 'Suburb saved.' ] );
...
if ( $app->request->isAjax and $app->request->type !== 'page' ) {
  ...
  $suburbId = $_GET['id'] ?? null;
  if ( empty( $suburbId ) ) respond_with( 'Bad request', 400 );
  $suburb = $db->getFirst( 'loc_suburbs', $suburbId );
  json_response( [ 'success' => true, 'data' => $suburb ] );

}


$form = new AppForm();
$suburbs = $db->table( 'loc_suburbs' )->orderBy('name')->getAll();
