<?php /* Admin Module - Settings SPA - Ncrs Sub Controller */
...
if ( $app->request->isPost ) {
  ...
  try {
    ...
    if ( $action === 'deleteNcr' )
    {
      ...
      $db->table('ch_ncrs')->delete( $id );
      
      $db->pdo->commit();

      json_response( [ 'success' => true, 'message' => 'Ncr deleted.' ] );
    ...
    //   $report['type_id'] = $_POST['type_id'] ?? null;
      ...
      $result = $db->table( 'ch_ncrs' )->save( $report, ['autoStamp' => true, 'user' => $uid ] );
      debug_log( $result, 'Save Ncr - Result: ' );
      
      $db->pdo->commit();

      json_response( [ 'success' => true, 'message' => 'Ncr saved.' ] );
...
if ( $app->request->isAjax and $app->request->type !== 'page' ) {
  ...
  $ncrId = $_GET['id'] ?? null;
  if ( empty( $ncrId ) ) respond_with( 'Bad request', 400 );
  $Ncrs = $db->getFirst( 'ch_ncrs', $ncrId );
  json_response( [ 'success' => true, 'data' => $Ncrs ] );

}


$form = new AppForm();
$ncrs = $db->table( 'ch_ncrs' )->orderBy('name')->getAll();
