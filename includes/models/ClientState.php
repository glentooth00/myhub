...
class ClientState
{
      ...
      if ($tcc->amount_used != $tcc->sumOfAmountsUsed) {
        ...
        if ($tcc->rollover > 0) {
          ...
          foreach ($coveredTrades as $trade) {
            ...
            $trade->year = (int) substr($trade->date, 0, 4);
            if ($trade->year > $tcc->year) {
              $coverAfterRolloverCheck += $tcc->allocs[$trade->trade_id];
            }
          } // foreach ( $coveredTrades )
  ...
  public function updateStateFor($client, $options = [])
  {
    ...
    $tradesThatNeedCover = [];
    foreach ($prepedTrades as $prepedTrade) {
      if ($prepedTrade->amount_covered < $prepedTrade->zar_sent)
        $tradesThatNeedCover[] = $prepedTrade;
    }
    ...
    foreach ($prepedTccsAffectingState as $tcc) {
      ...
      foreach ($tradesNotFullyCovered as $trade) {
        ...
        if ($fiaRemaining <= 0)
          break; // No more FIA cover available

        // NOTE: We rely on and trust the "amount available" value... make sure it's correct!
        if ($tcc->amount_available <= 0)
          break; // No more TCC cover available

        if ($trade->sda_fia == 'SDA')
          continue; // Skip SDA only trades
        if (!$trade->cover_required)
          continue;
        if ($trade->time < $tcc->time)
          continue;
        if ($trade->time > $tcc->expiresAt)
          continue;
    ...
    foreach ($clientPendingTCCs as $tcc) {
      $totalFiaPending += $tcc->amount_cleared_net;
    }

    $totalFiaDeclined = 0;
    $clientDeclinedTCCs = $clientModel->getDeclinedTccs($client->client_id, $theYear);
    foreach ($clientDeclinedTCCs as $tcc) {
      $totalFiaDeclined += $tcc->amount_cleared_net;
    }
